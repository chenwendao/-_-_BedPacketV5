 ...
 #region API声明
    ...
    string printerName,
         ...
         internal static extern bool SetForm(
          IntPtr phPrinter, [MarshalAs(UnmanagedType.LPTStr)] string pName,[MarshalAs(UnmanagedType.I4)] int level,ref FormInfo1 form);
         ...
         public static void AddCustomPaperSize(string PrinterName, string PaperName, float WidthInMm, float HeightInMm)
         {
             if (PlatformID.Win32NT == Environment.OSVersion.Platform)
             {
                 ...
                 if (OpenPrinter(PrinterName, out hPrinter, ref defaults))
                 {
                     ...
                     {
                         //如果Form存在删除之 
                         if (FormInPrinter(PrinterName, PaperName))
                             DeleteForm(hPrinter, PaperName);
                         //创建并初始化FORM_INFO_1 
...
//            {
//            //设置默认纸张为A4
//            ps=pd.PrinterSettings.PaperSizes[i];
//            if(ps.PaperName=="A4")
//            pd.DefaultPageSettings.PaperSize=ps;
//            }
//            //设置可见预览数量
//            printView.Columns=10;
//            printView.Rows=10;
//            Margins mar=new Margins(10,10,20,20);
//            pd.DefaultPageSettings.Margins=mar;
//            count=this.fraction.tbl.Rows.Count; //得到数据表行数
