...
left join SA_PreOrderMain k on j.id=k.id
        ...
        private DBTab getTabDb(string cPacketCode)
        {
            ...
            try
            {
                ...
                StringBuilder sb = new StringBuilder();
                sb.Append(@"select isnull(soex.cdefine5,cus.ccusabbname) ccusabbname,cus.ccusaddress,cus.ccusdefine4,so.csocode,sd.isosid,sd.irowno,a.cinvcode,isnull(soex.cdefine2,inv.cinvname) as cinvname,
                ...
                sb.Append(@" group by  isnull(soex.cdefine5,cus.ccusabbname),cus.ccusaddress,cus.ccusdefine4,so.csocode,sd.isosid,sd.irowno,a.                          cinvcode,isnull(soex.cdefine2,inv.cinvname),inv.cinvstd order by sd.irowno");
                DataTable dt = SqlAccess.ExecuteSqlDataTable(sb.ToString(), ufconn);
                string material = "";
                ...
                if (dt.Rows.Count == 1)
                    dr["Ordercode"] = dt.Rows[0]["csocode"].ToString() + "_" + dt.Rows[0]["irowno"].ToString();
                else
                ...
                for (int i = 0; i < (dt.Rows.Count > 9 ? 9 : dt.Rows.Count); i++)
                {
                    ...
                    if (dt.Rows.Count == 1)
                        ...
                        dr["cInvName" + (i + 1)] = String.Format("{0}_{1}", cInvname, dt.Rows[i]["irowno"].ToString());
