...
namespace CWD
{
    public partial class ucBrede : UserControl
    {
        ...
        private void tsbAddrow_Click(object sender, EventArgs e)
        {
            ...
            object oj = SqlAccess.ExecuteScalar(string.Format("select OperationId from sfc_operation where description like '{0}绣花'", row.DepCode == "3001" ? "床品" : row.DepCode == "2306" ? "毛巾" : "制衣"), conn);
            row.Op = new sfc_operation(Common.GetInt(oj), conn);
            if (row.ShowDialog() != DialogResult.OK)
                return;

            
            oj = SqlAccess.ExecuteScalar("select autoid from hy_modetails where IPROORDERAUTOID =" + row.MoDID + " and (cdescription='毛巾绣花' or cdescription='床品绣花' or cdescription='制衣绣花' or cdescription='床品印花') ", conn);
            if (oj == null || oj == DBNull.Value)
            {
                Common.ShowMessage("找不到对应工序委外加工单！",2);
                return;
            }
            HY_MODetails hd = new HY_MODetails(Common.GetInt(oj), conn);
            ...
            {                
                ...
                id.iQuantity = row.Qty;
                if (row.Qty < 0 && hd.fOutQuantity + row.Qty < 0)
                {
                    Common.ShowMessage("退料数不能多于累计发料数！",2);
                    return;
                }
                id.isFirstOP = false;
                id.isLastOP = false;
                ...
                sb.Append(" select mo.mocode,sortseq,mos.invcode,cinvname,cinvstd,cinvdefine1,ccomunitname from mom_orderdetail mos left join mom_order mo on mo.moid=mos.moid left join inventory inv on mos.invcode=inv.cinvcode left join computationunit unit on inv.ccomunitcode=unit.ccomunitcode where mos.modid="+row.MoDID);
                DataTable dt = SqlAccess.ExecuteSqlDataTable(sb.ToString(),conn);
                if(dt.Rows.Count>0)
                {
                    if (dt.Rows[0]["invcode"] != null && dt.Rows[0]["invcode"].ToString() != "")
                    {
                        id.InvCode = dt.Rows[0]["invcode"].ToString();
                    }
                    if (dt.Rows[0]["mocode"] != null && dt.Rows[0]["mocode"].ToString() != "")
                    {
                        id.MoCode = dt.Rows[0]["mocode"].ToString();
                    }
                    if (dt.Rows[0]["sortseq"] != null && dt.Rows[0]["sortseq"].ToString() != "")
                    {
                        id.MoSeq =Common.GetInt(dt.Rows[0]["sortseq"]);
                    }
                    if (dt.Rows[0]["cinvname"] != null && dt.Rows[0]["cinvname"].ToString() != "")
                    {
                        id.InvName = dt.Rows[0]["cinvname"].ToString();
                    }
                    if (dt.Rows[0]["cinvstd"] != null && dt.Rows[0]["cinvstd"].ToString() != "")
                    {
                        id.InvStd = dt.Rows[0]["cinvstd"].ToString();
                    }
                    if (dt.Rows[0]["cinvdefine1"] != null && dt.Rows[0]["cinvdefine1"].ToString() != "")
                    {
                        id.Color = dt.Rows[0]["cinvdefine1"].ToString();
                    }
                    if (dt.Rows[0]["ccomunitname"] != null && dt.Rows[0]["ccomunitname"].ToString() != "")
                    {
                        id.UnitName = dt.Rows[0]["ccomunitname"].ToString();
                    }
                }
                Issue.Items.Add(id);
        ...
        public void Init()
        {
            ...
            if (SRFlag)
            {
                sb.Append("  select max(ccode) from hy_receive reh left join hy_receivedetail red on reh.receiveid=red.receiveid left join sfc_moroutingdetail md on red.imoroutingdid=md.moroutingdid where md.description like '%绣花%'  and substring(ccode,1,6)=" + Common.SqlParm(String.Format("WF{0}", DateTime.Now.ToString("yyMM"))) + " and cCreateUser=" + Common.SqlParm(Public.userCode));
                object oj = SqlAccess.ExecuteScalar(sb.ToString(), conn);
                if (oj != null && oj != DBNull.Value)
                {
                    ShowSR(oj.ToString());
            ...
            {
                sb.Append(" select max(ccode) from hy_receive reh left join hy_receivedetail red on reh.receiveid=red.receiveid left join sfc_moroutingdetail md on red.imoroutingdid=md.moroutingdid where md.description like '%绣花%' and substring(ccode,1,6)='WS" + DateTime.Now.ToString("yyMM") + "' and cCreateUser=" + Common.SqlParm(Public.userCode));
                object oj = SqlAccess.ExecuteScalar(sb.ToString(), conn);
                if (oj != null && oj != DBNull.Value)
                {
                    ShowSR(oj.ToString());
        ...
        {
            fmBrede brede = (fmBrede)this.Parent;
            if (conn.State.ToString().ToLower() != "close")
                conn.Close();
            this.Dispose();
            brede.Close();
        }
        ...
        private void cwVendor_Click(object sender, EventArgs e)
        {
            ...
            if (input.ShowDialog() != DialogResult.OK)
                return;
            DataTable dt = SqlAccess.ExecuteSqlDataTable("SELECT Vendor.cVenCode,cVenAbbName  FROM Vendor INNER JOIN EKK..C_BredePlant plant ON Vendor.cVencode=plant.cVencode where plant.AutoID=" + input.cResult, conn);
            if (dt.Rows.Count>0)
            {
        ...
        private void cwDep_Click(object sender, EventArgs e)
        {
            ...
            if (oj != null)
            {
                string depcode=oj.ToString();
                cwDep.Text = oj.ToString();
                cwDep.Value = input.cResult;
                if (input.cResult == "3001")
                    Op = new sfc_operation(6, conn);
                else if (input.cResult == "2306")
                    Op = new sfc_operation(3, conn);
                else if (input.cResult == "25")
                    Op = new sfc_operation(19, conn);
                else
                    Common.ShowMessage("当前部门不支持绣花外发！",2);
            }
        }
        ...
        private void tsbPrev_Click(object sender, EventArgs e)
        {
            ...
            if (SRFlag)
            {
                if (lbSheetID.Text.Trim() == "")
                    sb.Append(" select max(ccode) from hy_issue ish left join hy_issuedetail isd on ish.issueid=isd.issueid left join sfc_moroutingdetail md on isd.imoroutingdid=md.moroutingdid where (md.description like '%绣花%' OR md.description like '%印花%') and substring(ccode,1,6)='WF" + DateTime.Now.ToString("yyMM") + "' and cCreateUser=" + Common.SqlParm(Public.userCode));                
                else
                    sb.Append(" select max(ccode) from hy_issue ish left join hy_issuedetail isd on ish.issueid=isd.issueid left join sfc_moroutingdetail md on isd.imoroutingdid=md.moroutingdid where(md.description like '%绣花%' OR md.description like '%印花%') and substring(ccode,1,6)=" + Common.SqlParm(lbSheetID.Text.Trim().Substring(0, 6)) + " and substring(ccode,7,4)<" + Common.SqlParm(lbSheetID.Text.Trim().Substring(6, 4)) + "and cCreateUser=" + Common.SqlParm(Public.userCode));                
                ...
                {
                    sb.Append(" select max(ccode) from hy_issue ish left join hy_issuedetail isd on ish.issueid=isd.issueid left join sfc_moroutingdetail md on isd.imoroutingdid=md.moroutingdid where (md.description like '%绣花%' OR md.description like '%印花%') and substring(ccode,3,8)<" + Common.SqlParm(lbSheetID.Text.Trim().Substring(2, 8)) + " and cCreateUser=" + Common.SqlParm(Public.userCode));                    
                    oj = SqlAccess.ExecuteScalar(sb.ToString(), conn);
                    if (oj != null && oj != DBNull.Value)
                    {
                        ShowSR(oj.ToString());
            ...
            {
                if (lbSheetID.Text.Trim() == "")
                    sb.Append(" select max(ccode) from hy_receive reh left join hy_receivedetail red on reh.receiveid=red.receiveid left join sfc_moroutingdetail md on red.imoroutingdid=md.moroutingdid where (md.description like '%绣花%' OR md.description like '%印花%') and substring(ccode,1,6)='WS" + DateTime.Now.ToString("yyMM") + "' and cCreateUser=" + Common.SqlParm(Public.userCode));
        ...
        private void tsbNext_Click(object sender, EventArgs e)
        {
            ...
            {
                if (lbSheetID.Text.Trim() == "")
                    sb.Append(" select min(ccode) from hy_issue ish left join hy_issuedetail isd on ish.issueid=isd.issueid left join sfc_moroutingdetail md on isd.imoroutingdid=md.moroutingdid where (md.description like '%绣花%' OR md.description like '%印花%') and substring(ccode,1,6)='WF" + DateTime.Now.ToString("yyMM") + "' and cCreateUser=" + Common.SqlParm(Public.userCode));
                else
                    sb.Append(" select min(ccode) from hy_issue ish left join hy_issuedetail isd on ish.issueid=isd.issueid left join sfc_moroutingdetail md on isd.imoroutingdid=md.moroutingdid where (md.description like '%绣花%' OR md.description like '%印花%') and substring(ccode,1,6)=" + Common.SqlParm(lbSheetID.Text.Trim().Substring(0, 6)) + " and substring(ccode,7,4)>" + Common.SqlParm(lbSheetID.Text.Trim().Substring(6, 4)) + " and cCreateUser=" + Common.SqlParm(Public.userCode));
                oj = SqlAccess.ExecuteScalar(sb.ToString(), conn);
                if (oj != null && oj != DBNull.Value)
            ...
            {
                if (lbSheetID.Text.Trim() == "")
                    sb.Append(" select min(ccode) from hy_receive reh left join hy_receivedetail red on reh.receiveid=red.receiveid left join sfc_moroutingdetail md on red.imoroutingdid=md.moroutingdid where (md.description like '%绣花%' OR md.description like '%印花%') and substring(ccode,1,6)='WS" + DateTime.Now.ToString("yyMM") + "' and cCreateUser=" + Common.SqlParm(Public.userCode));
                else
                    sb.Append(" select min(ccode) from hy_receive reh left join hy_receivedetail red on reh.receiveid=red.receiveid left join sfc_moroutingdetail md on red.imoroutingdid=md.moroutingdid where (md.description like '%绣花%' OR md.description like '%印花%')  and substring(ccode,1,6)=" + Common.SqlParm(lbSheetID.Text.Trim().Substring(0, 6)) + " and substring(ccode,7,4)>" + Common.SqlParm(lbSheetID.Text.Trim().Substring(6, 4)) + " and cCreateUser=" + Common.SqlParm(Public.userCode));                
                oj = SqlAccess.ExecuteScalar(sb.ToString(), conn);
                if (oj != null && oj != DBNull.Value)
        ...
        {     
            ...
            {
                string prnName="";
                List<C_PrinterSet> lstPrinter = C_PrinterSet.getModelPrinter("绣花", Public.userCode, conn);
